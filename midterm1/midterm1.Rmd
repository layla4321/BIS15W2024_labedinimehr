...
wolves %>%
  group_by(park) %>%
  select(packsize_aug) %>%
  summarise(sum_packsize=sum(packsize_aug, na.rm=T)) %>%
    arrange(desc(sum_packsize))
```
...
wolves %>%
  group_by(mort_all) %>%
  summarise(mort_all_total=sum(mort_all, na.rm=T)) %>%
  arrange(desc(mort_all_total))
```
...
ynp %>%
  group_by(biolyr) %>%
  filter(pack=="druid") %>%
  summarise(mean_size=mean(packsize_aug))
...
In the year 2001, there was an average of 37 in a pack making that the largest pack size. 

In 2010, the average was zero in a pack. This could possibly be due to an event that led to their extinction, or them migrating out of YNP. 
```{r}
ynp %>%
  group_by(biolyr) %>%
  filter(pack=="druid") %>%
  summarise(max_size=max(packsize_aug)) %>%
...
#ynp %>%
  #group_by(pack) %>%
  #summarise(max_biolyr=max(biolyr),
            #summarise(max_persisty1=max(persisty1))
