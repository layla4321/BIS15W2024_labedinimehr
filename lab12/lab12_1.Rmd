...
deserts %>%
  ...
  geom_point(shape=2)+ #you can experiment ith shapes
...
homerange %>% 
  ...
  geom_histogram(bin=30)+ #we can adjust the number of bins with the bins argument
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="blueviolet", alpha  =0.4, color = "black")+ #alpha is the transparency
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.mass)) +
  geom_histogram(aes(y = after_stat(density)), fill = "skyblue2", alpha = 0.4, color = "black")+
...
homerange %>%
  ...
  geom_histogram(color="black", fill="violetred", alpha= 0.3)+
...
homerange %>%
  ggplot(aes(x=log10.hra)) +
  geom_density(color="black", fill="violetred", alpha= 0.3)
  labs(title = "Distribution of Homerange")
```
...
#homerange %>%
  #ggplot(aes(x=log10.hra))+
  #geom_histogram(aes(y=after_stat(density), fill="violet", color="black", alpha= 0.3)+
  #geom_density(color="turquiose")+
  #labs(title = "Distribution of Homerange")
```
...
#log10.mass>1.75 & log10.mass>1.75 & lo10.mass<=2.75 ~ "medium",
...
```{r}
homerange %>%
  mutate(range_category=case_when(log10.hra<3.65 ~ "very small", log10.hra>=3.65 &log10.hra<=4.56 ~ "small",
