...
```
## here() starts at /Users/faramarzabedinimehr/Desktop/BIS15W2024_labedinimehr
```
...
Its tidy because each row contains an observation for each date. It is also long format for this reason. If it was wide, then you may have had the date column repeated like date1, date2, etc with that observation in that column and there are not multiple columns representing each variable. 
...
```r
average_enterococci_by_year <- sydneybeaches_long %>%
  group_by(site, year) %>%
  summarise(average_enterococci = mean(enterococci_cfu_100ml, na.rm = TRUE))
```
...
```r
average_enterococci_by_year_wide <- average_enterococci_by_year %>%
  pivot_wider(names_from = year, values_from = average_enterococci)
average_enterococci_by_year_wide
```
...
```r
most_polluted_beach_in_2013 <- average_enterococci_by_year %>%
  filter(year == "2013") %>% 
  arrange(desc(average_enterococci)) %>%
  top_n(1, average_enterococci)
most_polluted_beach_in_2013
```
